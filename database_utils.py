...
def valid_parent_login(username, password):
    ...
    pepper_and_salt = c.execute(q).fetchone()
    conn.close()
...
+------------+----------+----------+------+------------+-----------+-------+
| Teacher_ID | Username | Password | Salt | First Name | Last Name | Email |
+------------+----------+----------+------+------------+-----------+-------+
| INT        | TEXT     | INT      | INT  | TEXT       | TEXT      | TEXT  |
+------------+----------+----------+------+------------+-----------+-------+
"""
...
def valid_teach_login(username, password):
    return False


def valid_create_teacher(username, password, repeat_password, first_name, last_name, email):
    return False
